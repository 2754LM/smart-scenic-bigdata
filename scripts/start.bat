...
REM  17-container Docker stack (Hadoop HA + Spark + HBase + Kafka)
...
echo [2/5] Starting MySQL + ZooKeeper (3-node ensemble)...
...
REM ---------- 4. Start big-data + app stack ----------
echo [4/5] Starting remaining 13 services (Hadoop, Spark, HBase, Kafka, Backend)...
...
for /l %%i in (1,1,120) do (
    ...
    if !NAMENODE_OK! equ 1 if !BACKEND_OK! equ 1 if !HBASE_OK! equ 1 goto :all_ready
